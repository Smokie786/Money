...
var bot_token = '5489744314:AAE10SJadMyxk7DqSHzEHGLiAWHe-DF5l1g'; //YOUR BOT TOKEN HERE
var bot_name = 'MoneyEarn_HBot'; // Bot Name
...
const admin_id = 1574227974;
const admin_id2 = 1574227974;
var mongo_url = 'mongodb+srv://SHIVANSH:LG8KlwffICCMDTY3@cluster0.rqouy.mongodb.net/myFirstDatabase?retryWrites=true&w=majority'; // Your Mongo URL Here
const buttonsLimit = {
    window: 1000,
    limit: 1,
...
bot.hears(/^\/start (.+[1-9]$)/, async (ctx) => {
    try {
        ...
        if (admin.length == 0) {
            db.collection('admindb').insertOne({ admin: "admin", ref: 1, cur: 'INR', paychannel: '@MrKinGPaY', bonus: 0.1, minimum: 1, botstat: 'Active', withstat: 'ON', subwallet: 'NOT SET', MKEY: 'NOT SET', MID: 'NOT SET', channels: [] })
            ...
            if (flag == channel.length) {
                ...
                if (ctx.message.text == '⛔ Cancel'){
                  ...
                  ctx.replyWithMarkdown(
                    ...
                    await ctx.scene.leave('onWithdraw')
                    return 0;
                } else if (ctx.message.text > 50){
                  ctx.replyWithMarkdown(

                        '*⚠️ Maximum Withdrawal Is 50 ' + currency + '*', { reply_markup: { keyboard: [['💰 Balance'], ['👫 Invite', '🎁 Bonus', '🗂 Wallet'], ['💵 Withdraw', '📊 Statistics']], resize_keyboard: true } }
